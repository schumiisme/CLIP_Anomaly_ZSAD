...
class InCTRL(nn.Module):
    ...
    def forward(self, tokenizer, image: Optional[torch.Tensor] = None, text: Optional[torch.Tensor] = None, normal_list = None):
            ...
            for i in range(len(token)):
                ...
                vis_anomaly_maps.append(Fp_map.view(15, 15).unsqueeze(0))
                score = Fp_map.max(dim=0).values
                max_diff_score.append(score)

                image_feature = token[i].unsqueeze(0) / token[i].norm(dim=-1, keepdim=True)
                obj_type = text[i]
                normal_texts, anomaly_texts = get_texts(obj_type.replace('_', " "))
                pos_tokens = tokenizer(normal_texts).cuda()
                neg_tokens = tokenizer(anomaly_texts).cuda()
                pos_features = self.encode_text(pos_tokens)
                pos_features = pos_features / pos_features.norm(dim=-1, keepdim=True)
                neg_features = self.encode_text(neg_tokens)
            ...
            hl_score = self.diff_head.forward(holistic_map).squeeze(1)

            fg_score = torch.stack(max_diff_score)
            final_score = (hl_score + fg_score) / 2

            return final_score, {
                "anomaly_map": vis_anomaly_maps,
                "img_name": text,
                "orig_img": img
            }
